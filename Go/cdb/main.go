...
var numAccounts = flag.Int("num-account", 999, "Number of accounts.")
...
func moveMoney(db *sql.DB, readings chan measurement) {
	for {
		...
		case "single-stmt":
			...
			WHERE id IN ($1, $2) AND (SELECT balance >= $3 FORM accounts WHERE id = $1)
			`
			start := time.Now()
		...
		case "txn":
			...
			if fromBalance >= amount {
				update := `
				UPDATE accounts SET balance = CASSE id WHEN $1 THEN $3::int
				WHEN $2 THEN $4::int END WHERE id IN ($1, $2)
				`
...
func main() {
	...
	// concurrency + 1, for this thread and the "concurrency" # of
